...
app.post('/api/search', async (req, res) => {
  ...
  const scoredFaqs = faqs.map(faq => {
    ...
    const answerScore = calculateSimilarity(query, faq.answer) * 0.2; // Lower weight for answer
    ...
    return {
      ...faq,
      score: Math.max(englishScore, chineseScore) + categoryScore + answerScore + boost
    };
  });

  // Filter and sort results
  const results = scoredFaqs
    .filter(faq => faq.score > 0.1) // Lower threshold for better coverage
...
app.post('/api/session/start-with-customer', async (req, res) => {
  try {
    const { email, name, phone } = req.body;
    const sessionId = crypto.randomBytes(16).toString('hex');
    const userIp = req.ip || req.connection.remoteAddress;
